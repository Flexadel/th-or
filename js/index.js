...
var App = function () {
    ...
    return {
        showComment: function showComment() {
            $('.use__content .js-accordeon-toggler').click(function(evt) {
                evt.preventDefault();

                $(this).toggleClass('show');
                $(this).parents('.col__item').toggleClass('show');
            });
        },
        showReview: function showReview() {
            $('.content__item .js-accordeon-toggler').click(function(evt) {
                evt.preventDefault();

                $(this).toggleClass('show');
                $(this).parents('.content__item').toggleClass('show');
            });
        },
        scrollToTarget: function scrollToTarget(scrollSelector, speed) {
            var links = $(scrollSelector);
...
$('.reviews__slider').on('beforeChange', function (event, slick, currentSlide, nextSlide) {
    $('.content__item .js-accordeon-toggler').removeClass('show');
...
$('.answer__file').on('change', function () {
    ...
    // $('.file-name').text('Hoi');
    alert('ไฟล์ ' + filesName + ' ของคุณ ได้รับการอัพโหลดแล้ว');
});
...
$(document).ready(function () {
    ...
    setInterval(function () {
      ...
      if (counter == 1) {
        $(".users-count p").fadeOut(1100, function () {
          $(this).text('116 คนกำลังเยี่ยมชมอยู่ในขณะนี้');
        }).fadeIn();
        counter = 0;
      } else {
        $(".users-count p").fadeOut(1100, function () {
          $(this).text('มีคนดูอยู่ตอนนี้ ' + randNum + ' คน');
        }).fadeIn();
        counter = 1;
      }
    }, 6000);
...
function postDate(sa) {
    ...
    var sa = sa || 'dd.mm.yyyy',
        ...
        months, countryName = 'th',  // Мова для місяців. 
